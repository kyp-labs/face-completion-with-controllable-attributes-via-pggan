...
def numpy2tensor(use_cuda, x):
    """ Type conversion of numpy to tensor accoding to cuda use
    Args:
        use_cuda: flag for cuda use
        x:
    """
    var = torch.Tensor(torch.from_numpy(x))
    if use_cuda:
        var = var.cuda()
    return var
...
def rescale(img, from_range=[0.0, 255], to_range=[-1.0, 1.0]):
    """ rescale value of image in new reange
    Args:
        img: input image
        from_range: value range before change
        to_range: value range after change
    """
...
def normalize(img, from_range=[0.0, 255.0], to_range=[-1.0, 1.0]):
...
def denormalize(img, from_range=[-1.0, 1.0], to_range=[0.0, 255.0]):
